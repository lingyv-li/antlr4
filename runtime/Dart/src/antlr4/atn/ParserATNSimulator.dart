...
class ParserATNSimulator extends ATNSimulator {
  ...
  int adaptivePredict(
    ...
    try {
      ...
      if (debug)
        log("DFA after predictATN: " + dfa.toString(parser.getVocabulary()));
      return alt;
    } finally {
  ...
  void predicateDFAState(DFAState dfaState, DecisionState decisionState) {
    ...
    BitArray altsToCollectPredsFrom =
...
//			log("reach="+reach+", "+reach.conflictingAlts, level: Level.SEVERE.value);
  ...
  List<SemanticContext> getPredsForAmbigAlts(
      BitArray ambigAlts, ATNConfigSet configs, int nalts) {
...
//		}
  ...
  List<PredPrediction> getPredicatePredictions(
      BitArray ambigAlts, List<SemanticContext> altToPred) {
...
//		log(Arrays.toString(altToPred)+"->"+pairs);
   ...
   */
  BitArray evalSemanticContext(List<PredPrediction> predPredictions,
      ParserRuleContext outerContext, bool complete) {
    BitArray predictions = new BitArray();
    for (PredPrediction pair in predPredictions) {
      if (pair.pred == SemanticContext.NONE) {
        predictions.setBit(pair.alt);
        if (!complete) {
          break;
        }
        continue;
...
//          assert(newDepth > int.MIN_VALUE);
    ...
    if (collectPredicates && inContext) {
      if (fullCtx) {
        ...
        bool predSucceeds = evalSemanticContextOne(
            pt.getPredicate(), _outerContext, config.alt, fullCtx);
  ...
  /**
   * Gets a {@link BitArray} containing the alternatives in {@code configs}
   ...
   * conflicting subsets, this method returns an empty {@link BitArray}.
   */
  BitArray getConflictingAlts(ATNConfigSet configs) {
    List<BitArray> altsets =
   ...
   */
  BitArray getConflictingAltsOrUniqueAlt(ATNConfigSet configs) {
    BitArray conflictingAlts;
    if (configs.uniqueAlt != ATN.INVALID_ALT_NUMBER) {
      conflictingAlts = new BitArray();
      conflictingAlts.setBit(configs.uniqueAlt);
    } else {
  ...
  void reportAttemptingFullContext(DFA dfa, BitArray conflictingAlts,
