...
//		System.out.println("visitState "+p.stateNumber);
    ...
    switch (transition.type) {
      ...
      case TransitionType.NOT_SET:
        if (!transition.matches(
            getInputStream().LA(1), Token.MIN_USER_TOKEN_TYPE, 65535)) {
          recoverInline();
        }
        matchWildcard();
        break;
  ...
  void recover(RecognitionException e) {
    int i = getInputStream().index;
    getErrorHandler().recover(this, e);
    if (getInputStream().index == i) {
      ...
      if (e is InputMismatchException) {
        ...
        Token errToken = getTokenFactory().create(
            expectedTokenType,
            tok.text,
            new Pair(tok.tokenSource, tok.tokenSource.getInputStream()),
