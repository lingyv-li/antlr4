...
import 'atn/ArrayPredictionContext.dart';
import 'atn/PredictionContextCache.dart';
...
abstract class PredictionContext {
  ...
  static int calculateHashCode(List<PredictionContext> parents,
   ...
   * @param mergeCache
   */
  static PredictionContext mergeSingletons(SingletonPredictionContext a,
    ...
    if (a.returnState == b.returnState) { // a == b
      PredictionContext parent = merge(
          a.parent, b.parent, rootIsWildcard, mergeCache);
      ...
      if (a == b || (a.parent != null &&
          a.parent.equals(b.parent))) { // ax + bx = [a,b]x
   ...
   */
  static PredictionContext mergeArrays(ArrayPredictionContext a,
      ArrayPredictionContext b,
      bool rootIsWildcard,
      Map<Pair<PredictionContext, PredictionContext>,
          PredictionContext> mergeCache) {
    if (mergeCache != null) {
      PredictionContext previous = mergeCache[Pair(a, b)];
      if (previous != null) return previous;
    ...
    List<int> mergedReturnStates =
    List(a.returnStates.length + b.returnStates.length); // TODO Will it grow?
    var mergedParents =
  ...
  static void combineCommonParents(List<PredictionContext> parents) {
    Map<PredictionContext, PredictionContext> uniqueParents = new Map<
        PredictionContext,
        PredictionContext>();
  ...
  static PredictionContext getCachedContext(PredictionContext context,
    ...
    bool changed = false;
    var parents = List<PredictionContext>(context.size());
    for (int i = 0; i < parents.length; i++) {
      PredictionContext parent = getCachedContext(
          context.getParent(i), contextCache, visited);
      if (changed || parent != context.getParent(i)) {
        if (!changed) {
          parents = List<PredictionContext>(context.size());
    ...
    if (parents.length == 0) {
      ...
      updated =
      new ArrayPredictionContext(parents, arrayPredictionContext.returnStates);
    }
...
//	}
  ...
  static void getAllContextNodes_(PredictionContext context,
  ...
  // FROM SAM
  List<String> toStrings(Recognizer recognizer, PredictionContext stop,
      int currentState) {
    List<String> result = [];
...
class SingletonPredictionContext extends PredictionContext {
  ...
  SingletonPredictionContext(PredictionContext this.parent, this.returnState)
      :super(
      parent != null ? PredictionContext.calculateHashCode(
          [parent], [returnState]) : PredictionContext
          .calculateEmptyHashCode()) {
    assert (this.returnState != ATNState.INVALID_STATE_NUMBER);
  }

  static SingletonPredictionContext create(PredictionContext parent,
      int returnState) {
